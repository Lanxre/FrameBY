...
export function useCreateUniversityDepartment() {
  ...
  const create = async (data: CreateUniversityDepartmentData): Promise<boolean> => {
    isLoading.value = true
    errorMessage.value = ''
    isSuccess.value = false

    try {
      const response = await $api<{ department: CreatedUniversityDepartment }>('/university-departments', {
        method: 'POST',
        body: data
      })
